...
# Note: This rmarkdown is named 5bc-OutcomePaper because it aligns with output from either:
#       4b-ReadRun.R and saved in Mplus_Automatr_Results.RData
#       4c-Wave1Sample.R and saved in Mplus_Wave1Sample_Results.RData
...
bly.est <- inner_join(tx.covs.results$`Bully Perpetration`, tx.covs.results$`Cyberbully Perpetration`, by = "predictor", suffix = c(".bly", ".cbl")) %>%
   inner_join(tx.covs.results$`Peer Victimization`, by = "predictor")

bly.map <- data.frame(col_keys = names(bly.est),
                            top = c("predictor", rep(c("Bullying", "Cyberbullying", "Peer Victimization"), each = 2)),
