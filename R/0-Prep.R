...
ds1 <- dsclean %>%
  ...
  select(StudentID, School = SCHOOL_NAME_ALL,                      # Identifiers
         ...
         starts_with("AGE_"), Grade, one_of(DemoVars),             # Demographics; GRADE_W1 = RGRADE_R1 (using Kelly's Grade variable instead)
...
ds1 <- ds1 %>%
  ...
  select(StudentID, School, Tx, starts_with("HasASurvey_"),
         Grade:SexOr, starts_with("AGE_"), everything(), -starts_with("CONDITION"))
...
ds1 <- ds1 %>%
  ...
  select(StudentID:SexOr, Female:LGBQxTx, everything())
...
subby <- c("School", "Grade", "Transgender", "Gender", "Race", "SexOr", "Nominated_Trusted_Adult",
           "Sexual_Harassment", "Forced_Sexual_Contact", "Sexual_Violence","HNC", "Dismissive")
