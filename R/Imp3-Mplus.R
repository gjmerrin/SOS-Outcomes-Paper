...
## TO-DO:
# Use ModInd to help finalize Implementation models, then add other implementation variables
# Make sure exposure variables are correlated in the same way in both Exposure only model and ImpEx model
...
ds3 <- readRDS("SoS Data and Codes/ds3.rds")
...
ds4 <- ds3 %>%
  ...
  select(ID = StudentID, School:Grade, TransG = Transgender, Gender:Asian, MultiR = Multiracial, Indigen = Indigenous,
         ...
         starts_with(c("TotAdultNoms", "Trusted_Adult",
                       ...
                       "Bullying_Score", "Cyberbullying_Score", "Peer_Victimization_Score")),
...
impexpout = c(NCPS = "SH Perpetration", NCVS = "SH Victimization", CPS = "FSC Perpetration", CVS = "FCS Victimization", 
              HOPS = "HNC Perpetration", HOVS = "HNC Victimization", DSVS = "SV Dismissiveness", #CYPS = "Cybersex Perpetration",
              # GWBS = "General Well-Being", SHAS = "SH Attitudes", SHIS = "SH Help Intent", BLLY = "Bully Perpetration",
              # CBLY = "Cyberbully Perpetration", PEER = "Peer Victimization",
              TA = "Trusted Adult")
impexpout.len <- length(impexpout)
...
imp.sntx <- map2(.x = impexpout[-impexpout.len], .y = names(impexpout)[-impexpout.len],
                    ~mplusObject(
  rdata = ds4, #[ds4$Tx==1,]
  usevariables = c("ID", "School", paste0("AEXS", 2:4), paste0("PEXS", 2:4),
...
imp.fit <- map_dfr(imp.mods, ~mplus_fit(.x, digits = 2), .id = "outcome") %>%
  outcome_shortcut(remove = "_exp_imp.out", recodes = impexpout[-impexpout.len], tolower = TRUE) %>%
  select(Outcome = outcome, chisq, df, p = pvalue, CFI = cfi, RMSEA = rmsea, `90P CI` = rmsea.ci, SRMR = srmr)

## Fixed effect estimates
imp.est <- map2(.x = imp.mods[c(6,7,1,2,4,5,3)], #adjusting order to sync with impexpout rather than be alphabetic
                .y = names(impexpout)[-impexpout.len],
...
imp.results <- list(Models = imp.mods,
                    ...
                    R2 = map_dfr(.x = imp.mods[-impexpout.len], ~.x$parameters$r2, .id = "outcome") %>%
                      outcome_shortcut(remove = "_exp_imp.out", recodes = impexpout[-impexpout.len], tolower = TRUE),
                    Warn_Error = map_dfr(.x = imp.mods[-impexpout.len],
...
# --------   Saving Results    ------------
save(impexpout, impexpout.len, ds4,
     impexp.preds, impexp.mods, impexp.fit, impexp.est,
     imp.mods, imp.results,
