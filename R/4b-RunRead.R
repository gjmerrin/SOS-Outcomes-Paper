...
mplusoutcomes = c(NCPS = "SH Perpetration", NCVS = "SH Victimization", CPS = "FSC Perpetration", CVS = "FCS Victimization", 
                  ...
                  CBLY = "Cyberbully Perpetration", PEER = "Peer Victimization", TA = "Trusted Adult")
...
lgm.sntx <- map2(.x = mplusoutcomes[-15], .y = names(mplusoutcomes)[-15], # drop TA, which has a separate model
                       ~mplusObject(
                         rdata = mpluswide2,
...
## Trusted Adult Model
ta.lgm.sntx <- mplusObject(
  rdata = mpluswide2,
  usevariables = c("ID", "School", paste0("TA_", 1:4)),
  TITLE = "Trusted Adult - LGM;",
  VARIABLE = "CATEGORICAL = TA_1-TA_4;
              IDVARIABLE = ID;
              CLUSTER = School;",
  ANALYSIS = "TYPE = COMPLEX;
              ESTIMATOR = MLR;
              LINK = LOGIT;",
  MODEL = "i s | TA_1@0 TA_2@1 TA_3@2 TA_4@3;
  [TA_1$1-TA_4$1@0]; 
  [i];",
...
ta.lgm.run <- mplusModeler(ta.lgm.sntx,
                           dataout = "Mplus/Automatr/LGM/TA_LGM.dat",
                           modelout = "Mplus/Automatr/LGM/TA_LGM.inp",
                           run = 1, check = TRUE, hashfilename = TRUE,
                           writeData = "always")
...
lgm.fit <- map_dfr(lgm.mods[1:14], ~mplus_fit(.x, digits = 2), .id = "outcome") %>%
  outcome_shortcut(remove = "_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE)
# lgm.fit will go into supplemental materials


lgm.est <- map_dfr(lgm.mods[1:14], ~mplus_est(.x, params = c("Means", "^Variances", "WITH"), std = "unstandardized"), .id = "outcome") %>%
  outcome_shortcut(remove = "_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE) %>%
...
lgm.results <- list(Models = lgm.mods[1:14],
                       Fit = lgm.fit,
                       Estimates = lgm.est,
                       R2 = map_dfr(.x = lgm.mods[1:14], ~.x$parameters$r2, .id = "outcome") %>%
                         outcome_shortcut(remove = "_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE),
                       Warn_Error = map_dfr(.x = lgm.mods[1:14],
...
quad.sntx <- map2(.x = mplusoutcomes[-15], .y = names(mplusoutcomes)[-15], # drop TA, which has a separate model
                 ~mplusObject(
                   rdata = mpluswide2,
                   usevariables = c("ID", "School", paste0(.y, "_", 1:4)),
...
quad.fit <- map_dfr(quad.mods[1:14], ~mplus_fit(.x, digits = 2), .id = "outcome") %>%
  outcome_shortcut(remove = "_quad.out", recodes = mplusoutcomes[1:14], tolower = TRUE)

## comparing linear and quadratic
lq.comp <- map2_dfr(.x = lgm.mods[-15], .y = quad.mods,
...
ml.lgm.sntx <- map2(.x = mplusoutcomes[-15], .y = names(mplusoutcomes)[-15], # drop TA, which has a separate model
...
ml.lgm.fit <- map_dfr(ml.lgm.mods[1:14], ~mplus_fit(.x, digits = 2, ml = TRUE), .id = "outcome") %>%
  outcome_shortcut(remove = "_ml_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE)

ml.lgm.est <- map_dfr(ml.lgm.mods[1:14], ~mplus_est(.x, params = c("Means", "^Variances", "WITH"), std = "unstandardized"), .id = "outcome") %>%
  outcome_shortcut(remove = "_ml_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE) %>%
  select(outcome, estimate, parameter) %>%
  spread(parameter, estimate)
...
####   Compiling results objects into one object    ####
ml.lgm.results <- list(Models = ml.lgm.mods[1:14],
                    Fit = ml.lgm.fit,
                    Estimates = lgm.est,
                    R2 = map_dfr(.x = ml.lgm.mods[1:14], ~.x$parameters$r2, .id = "outcome") %>%
                      outcome_shortcut(remove = "_ml_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE),
                    Warn_Error = map_dfr(.x = ml.lgm.mods[1:14],
                                         ~data.frame(errors = length(.x$errors),
                                                     warnings = length(.x$warnings)),
                                         .id = "outcome") %>%
                      outcome_shortcut(remove = "_ml_lgm.out", recodes = mplusoutcomes[1:14], tolower = TRUE))
...
only.sntx <- map2(.x = mplusoutcomes[-15], .y = names(mplusoutcomes)[-15], # exclude TA, which has a separate model
                        ~mplusObject(
                          rdata = mpluswide2,
...
## Trusted Adult Model
ta.only.sntx <- mplusObject(
  rdata = mpluswide2,
  usevariables = c("ID", "School", "Tx", paste0("TA_", 1:4)),
  TITLE = "Trusted Adult - Tx only;",
  VARIABLE = "CATEGORICAL = TA_1-TA_4;
  IDVARIABLE = ID;
  CLUSTER = School;",
  ANALYSIS = "TYPE = COMPLEX;
...
ta.only.run <- mplusModeler(ta.only.sntx,
                            dataout = "Mplus/Automatr/TX ONLY/TA_TX_ONLY.dat",
                            modelout = "Mplus/Automatr/TX ONLY/TA_TX_ONLY.inp",
                            run = 1, check = TRUE, hashfilename = TRUE,
                            writeData = "always")
...
## Model Fit
tx.only.fit <- map_dfr(tx.only.mods[1:14], ~mplus_fit(.x, digits = 2), .id = "outcome") %>%
  outcome_shortcut(remove = "_tx_only.out", recodes = mplusoutcomes[1:14], tolower = TRUE)

## Fixed effect estimates
tx.only.est <- map_dfr(.x = tx.only.mods[1:14],
...
####   Compiling results objects into one object    ####
tx.only.results <- list(Models = tx.only.mods[1:14],
                        Fit = tx.only.fit,
                        Estimates = tx.only.est,
                        R2 = map_dfr(.x = tx.only.mods[1:14], ~.x$parameters$r2, .id = "outcome") %>%
                          outcome_shortcut(remove = "_tx_only.out", recodes = mplusoutcomes[1:14], tolower = TRUE),
                        Warn_Error = map_dfr(.x = tx.only.mods[1:14],
                                             ~data.frame(errors = length(.x$errors),
                                                         warnings = length(.x$warnings)),
                                             .id = "outcome") %>%
                          outcome_shortcut(remove = "_tx_only.out", recodes = mplusoutcomes[1:14], tolower = TRUE))
...
covs.sntx <- map2(.x = mplusoutcomes[-15], .y = names(mplusoutcomes)[-15], # drop TA, which has a separate model
                        ~mplusObject(
                          rdata = mpluswide2,
...
ta.covs.sntx <- mplusObject(
  ...
  usevariables = c("ID", "School", "Tx",
                   ...
                   paste0("TA_", 1:4)),
  TITLE = "Trusted Adult - Tx with no interactions;",
  VARIABLE = "CATEGORICAL = TA_1-TA_4;
  IDVARIABLE = ID;
  CLUSTER = School;",
  ANALYSIS = "TYPE = COMPLEX;
...
ta.covs.run <- mplusModeler(ta.covs.sntx,
                           dataout = "Mplus/Automatr/TX COVS/TA_TX_NOINT.dat",
                           modelout = "Mplus/Automatr/TX COVS/TA_TX_NOINT.inp",
                           run = 1, check = TRUE, hashfilename = TRUE,
                           writeData = "always")
...
tx.covs.fit <- map_dfr(tx.covs.mods[1:14], ~mplus_fit(.x, digits = 2), .id = "outcome") %>%
  outcome_shortcut(remove = "_tx_covs.out", recodes = mplusoutcomes[1:14], tolower = TRUE)

# in a list for joining with estimates
tx.covs.fitlong <- map(tx.covs.mods[1:14],
...
## Fixed effect estimates
tx.covs.est <- map(.x = tx.covs.mods[1:14],
...
tx.covs.re <- map(.x = tx.covs.mods[1:14],
...
names(tx.covs.results) <- names(tx.covs.results) %>% str_remove(., "_tx.covs.out") %>%
  toupper() %>% recode(., !!!mplusoutcomes[1:14])
tx.covs.results <- tx.covs.results[mplusoutcomes[1:14]]
...
save(mplusoutcomes, lgm.mods, lgm.results, ml.lgm.results,
     ...
     tx.only.results, unctx.fit, unctx.est,
